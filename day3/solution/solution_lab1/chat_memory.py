...
def trim_messages(messages: list[Message], max_turns: int) -> list[Message]:
    ...
    for msg in messages:
        if msg.get("role") == "system":
            system_messages.append({"role": "system", "content": str(msg.get("content", ""))})
        else:
            # Keep as-is; tests only rely on role/content keys.
    ...
    return list(system_messages) + conversation[-keep_n:]
